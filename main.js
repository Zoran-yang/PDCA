...
async function intialNotification() { 
  ...
  timeId = setInterval(() => {showNotification(notifier, path, BrowserWindow)},  minuteSetting * 60 * 1000)
  notifier.notify({
    title: 'App started',
...
app.on('ready', () => { // launch app when app is ready
  createTray(timeId, settings, dialog, app, BrowserWindow, Menu, path, Tray, notifier, showNotification)
})
